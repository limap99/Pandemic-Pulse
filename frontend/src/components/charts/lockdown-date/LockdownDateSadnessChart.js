...
const LockdownDateSadnessChart = ({data}) => {
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Late Lockdown',
        data: data.filter(item => item.lockdown_status === 'Late').map(item => item.sadness_trend), // Assuming 'value' is the float value
        borderColor: 'teal',
        backgroundColor: 'teal',
      },
      {
        label: 'Early Lockdown',
        data: data.filter(item => item.lockdown_status === 'Early').map(item => item.sadness_trend), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
      }
    ],
...
export default LockdownDateSadnessChart;
