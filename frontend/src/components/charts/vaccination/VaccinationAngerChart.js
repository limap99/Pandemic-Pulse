...
const VaccinationAngerChart = ({data, country}) => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        label: 'Anger Trend',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.anger_trend), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default VaccinationAngerChart;