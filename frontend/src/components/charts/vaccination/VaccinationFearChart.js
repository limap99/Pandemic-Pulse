...
const VaccinationFearChart = ({data, country}) => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        label: 'Fear Trend',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.fear_trend), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default VaccinationFearChart;