...
const VaccinationJoyChart = ({data, country}) => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        label: 'Joy Trend',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.joy_trend), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default VaccinationJoyChart;