...
const CovidCasesJoyChart = ({data, country}) => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        label: 'Joy Delta',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.joy_delta), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default CovidCasesJoyChart;
