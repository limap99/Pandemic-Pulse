...
const CovidCasesSadnessChart = ({data, country}) => {
    ...
    datasets: [
      {
        ...
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.covid_cases_delta), // Assuming 'value' is the float value
        borderColor: 'teal',
      ...
      {
        label: 'Sadness Delta',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.sadness_delta), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default CovidCasesSadnessChart;
