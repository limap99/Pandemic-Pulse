...
const CovidCasesFearChart = ({data, country}) => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      {
        label: 'Fear Delta',
        data: data.filter(item => item.country_ID === `${country}`).map(item => item.fear_delta), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
        yAxisID: 'y1'
      }
...
export default CovidCasesFearChart;
