...
const LockdownTypeJoyChart = ({data}) => {
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: 'Full Lockdown',
        data: data.filter(item => item.lockdown_status === 'FULL').map(item => item.joy_trend), // Assuming 'value' is the float value
        borderColor: 'teal',
        backgroundColor: 'teal',
      },
      {
        label: 'Partial Lockdown',
        data: data.filter(item => item.lockdown_status === 'Partial').map(item => item.joy_trend), // Assuming 'value' is the float value
        borderColor: 'red',
        backgroundColor: 'red',
      }
    ],
...
export default LockdownTypeJoyChart;
